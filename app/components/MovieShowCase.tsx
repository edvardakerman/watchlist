...
export default function MovieShowCase({ movies, title }: MovieProps) {
    ...
    return (
        <div className="py-10">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 mt-8 gap-6">
                {topTwelve.map((movie) => (
                    <div className="relative group">
                        <Link href={`/home/movie/${movie.id}`}>
                            <Image className="hover:brightness-50" title={movie.title} width={100} height={100} alt="movie poster" src={`https://image.tmdb.org/t/p/w500/` + `${movie.poster_path}`}></Image>
                        </Link>
